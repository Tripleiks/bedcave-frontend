...
const providers: CloudProvider[] = [
  {
    ...
    region: "West Europe",
    uptime: "99.99%",
    icon: <svg viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor"><path d="M5.483 18.3L24 5.7V0L0 12.9l5.483 5.4zm13.434-9.9l-5.483 9.9H24V8.4h-5.083zM0 24h24l-5.483-5.7H5.483L0 24z"/></svg>
  },
  {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-2">
      <div className="relative rounded-lg overflow-hidden border border-[#1e293b] bg-[#0a0a0f]">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 bg-[#13131f] border-b border-[#1e293b]">
          <div className="flex items-center gap-3">
            <Server className="w-5 h-5 text-[#00d4ff]" />
            <span className="font-mono text-sm text-[#64748b]">$ cloud-status --region=eu --monitor</span>
          </div>
          <div className="flex items-center gap-4">
            <span className="font-mono text-xs text-[#64748b]">{currentTime.toLocaleTimeString()}</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-[#1e293b]">
          {providers.map((provider) => (
            <motion.div
              ...
              className="bg-[#0a0a0f] p-5 relative overflow-hidden group"
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-mono text-sm font-bold" style={{ color: provider.color }}>
                      {provider.shortName}
                    </span>
                  </div>
                ...
                {/* Status Text */}
                <div className="font-mono text-xs text-[#64748b] mb-3 flex items-center gap-1.5">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-mono text-xs text-[#64748b]">{provider.region}</span>
                  </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-5 py-2.5 bg-[#13131f] border-t border-[#1e293b]">
          <div className="flex items-center gap-6">
            <span className="font-mono text-xs text-[#64748b]">Region: <span className="text-[#00d4ff]">Europe</span></span>
            <span className="font-mono text-xs text-[#64748b]">Last check: <span className="text-[#39ff14]">2s ago</span></span>
          </div>
          <div className="flex items-center gap-1.5">
            <Zap className="w-4 h-4 text-[#ffbe0b]" />
            <span className="font-mono text-xs text-[#ffbe0b]">LIVE</span>
          </div>
        </div>
      </div>
    </div>
